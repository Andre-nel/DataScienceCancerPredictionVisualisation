...
from diagnosis_encoder import DiagnosisEncoder, DataFrameSelector
...
def preprocess_cancer_data(data, pipeline):
    ...
    selector.fit(data_encoded)
    data_selected = selector.transform(data_encoded)

    data_scaled = pipeline.named_steps["scaler"].fit_transform(data_selected)
    ...
    diagnosis_column = pd.DataFrame(data_encoded[:, 0], columns=["diagnosis"])
...
if __name__ == "__main__":
    ...
    raw_data_path = projectRoot / "data/raw/cancerData.csv"
    preprocessed_data_path = projectRoot / "data/preprocessed/cancer_data.csv"
    pipeline_path = projectRoot / "src/models/pickled/preprocessor_pipeline.pkl"

    data = pd.read_csv(raw_data_path)
    data.drop(columns=["Unnamed: 32", "id"], inplace=True)

    preprocessor_pipeline = create_pipeline(data)
    preprocessed_data = preprocess_cancer_data(data, preprocessor_pipeline)
